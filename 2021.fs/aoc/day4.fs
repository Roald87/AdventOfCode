...
module Day4 =
    ...
    let emptyRow = List.filter (fun x -> x <> "") >> List.length >> (=) 0

    let emptyRows = List.map emptyRow >> List.exists (fun x -> x = true)
    ...
    let rowScore = List.filter (fun x -> x <> "") >> List.map Convert.ToInt32

    let totalBoardScore = List.map rowScore >> List.map List.sum >> List.sum

    let firstToWin draws boards =
        let rec checkBingo (numbersToDraw: list<string>) boards =
            let newBoard = boards |> List.map (removeDrawnNumber numbersToDraw.Head)

            let bingo =
                newBoard |> List.tryFind (fun x -> emptyRows x || emptyColumns x)
