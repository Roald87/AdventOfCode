...
module Day12 =
    ...
    let trajectories input : Map<string, list<string>>=
        ...
        |> List.map (fun (from, toos) -> (from, (List.filter (fun towards -> from <> towards) toos)))
        |> Map    
    ...
    let part1 input =
        ...
        let rec countNextPaths origin seen =
            let newSeen = 
            ...
            for target in caveMap.[origin] do
                if target = "end" then
                    ...
                    nPaths <- countNextPaths target newSeen
                else
                    ()
    
            nPaths
    
        countNextPaths "start" Set.empty